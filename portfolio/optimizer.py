...
class PortfolioOptimizer:
    ...
    def prepare_returns_data(self, price_data: Dict[str, List[Dict[str, Any]]]) -> pd.DataFrame:
    ...
    def optimize_portfolio(self, price_data: Dict[str, List[Dict[str, Any]]], 
        ...
        try:
            ...
            if SCIPY_AVAILABLE and objective in ['sharpe', 'min_volatility', 'max_return']:
    ...
    def _scipy_optimization(self, returns_df: pd.DataFrame, symbols: List[str], 
    ...
    def _simple_optimization(self, returns_df: pd.DataFrame, symbols: List[str]) -> Dict[str, Any]:
        ...
        for symbol in symbols:
            ...
            sharpe = (annual_return - self.risk_free_rate) / annual_risk if annual_risk > 0 else 0
        ...
        for metric in asset_metrics:
            if metric['sharpe'] > 0 and total_sharpe > 0:
                weight = max(0.1, metric['sharpe'] / total_sharpe)  # Minimum 10% if included
    ...
    def _calculate_comprehensive_metrics(self, returns_df: pd.DataFrame, 
                                       weights: np.ndarray) -> PortfolioMetrics:
        ...
        beta = 1.0  # Simplified
        alpha = annual_return - (self.risk_free_rate + beta * (annual_return - self.risk_free_rate))
    ...
    def calculate_risk_metrics(self, portfolio_holdings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        weights = {h['symbol']: h['market_value'] / total_value for h in portfolio_holdings}
        
        # Concentration risk
        max_weight = max(weights.values())
        concentration_score = "High" if max_weight > 0.3 else "Medium" if max_weight > 0.2 else "Low"
        
        # Sector diversification (simplified)
        unique_symbols = len(weights)
        diversification_score = "High" if unique_symbols >= 8 else "Medium" if unique_symbols >= 5 else "Low"
        
        # Risk-adjusted return potential
        avg_return = sum(h.get('expected_return', 0.08) * weights[h['symbol']] 
    ...
    def _calculate_risk_score(self, weights: Dict[str, float], holdings: List[Dict[str, Any]]) -> str:
        ...
        high_vol_weight = sum(weights[h['symbol']] for h in holdings 
                             if h.get('volatility', 0.2) > 0.3)
    ...
    def suggest_rebalancing(self, current_holdings: List[Dict[str, Any]], 
        ...
        # Current weights
        current_weights = {h['symbol']: h['market_value'] / total_portfolio_value 
        ...
        for symbol in set(list(current_weights.keys()) + list(target_weights.keys())):
            ...
            if abs(weight_diff) > 0.05:  # Only suggest if difference > 5%
                ...
                suggestions.append({
                    ...
                    'priority': 'High' if abs(weight_diff) > 0.15 else 'Medium' if abs(weight_diff) > 0.10 else 'Low'
                })
        
        # Sort by priority and dollar amount
        priority_order = {'High': 3, 'Medium': 2, 'Low': 1}
        suggestions.sort(key=lambda x: (priority_order[x['priority']], x['dollar_amount']), reverse=True)
