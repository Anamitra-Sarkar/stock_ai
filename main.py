...
def get_dashboard_data():
    ...
    print(
        f"📊 Dashboard: {real_data_count}/{len(all_stock_analysis)} stocks have REAL data"
    )
...
def ask_assistant():
    ...
    return jsonify(
        {
            "response": "I can analyze stocks across Technology, Healthcare, Finance, Energy, Consumer, Industrial, and Telecom sectors. Try asking about a specific stock or sector!"
        }
    )
...
def analyze_portfolio_risk():
    ...
    try:
        ...
        position_risks = risk_manager.analyze_position_risk(
            holdings, sum(h.get("market_value", 0) for h in holdings)
        )
...
def handle_subscribe(data):
    ...
    if success:
        ...
        emit(
            "subscription_error",
            {"error": "Failed to subscribe to symbols", "symbols": symbols},
        )
...
def handle_unsubscribe(data):
    ...
    emit(
        ...
        {"room": room, "message": f"Unsubscribed from room {room}"},
    )
...
def handle_get_prediction(data):
    ...
    try:
        ...
        emit(
            "prediction_result",
            {"ticker": ticker, "prediction": prediction, "timestamp": time.time()},
        )
    except Exception as e:
        emit("prediction_error", {"ticker": ticker, "error": str(e)})
...
if __name__ == "__main__":
    ...
    print(
        f"🏗️  Enterprise features: WebSocket, ML, Portfolio Optimization, Risk Management"
    )
    print(f"📡 WebSocket streaming: Active")
    print(
        f"🗄️  Database: {'Connected' if db_manager._initialized else 'Using fallback'}"
    )
