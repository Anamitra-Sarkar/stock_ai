...
class StreamingManager:
    ...
    def _deterministic_hash(self, symbol: str, seed: int = 0) -> float:
        ...
        hash_val = int(
            hashlib.md5(combined.encode(), usedforsecurity=False).hexdigest()[:8], 16
        )
        return hash_val / (2**32 - 1)

    def _deterministic_gauss(
        self, symbol: str, mean: float = 0, sigma: float = 1, seed: int = 0
    ) -> float:
    ...
    def _deterministic_randint(
        self, symbol: str, min_val: int, max_val: int, seed: int = 0
    ) -> int:
    ...
    def _deterministic_uniform(
        self, symbol: str, min_val: float, max_val: float, seed: int = 0
    ) -> float:
    ...
    def subscribe_to_symbol(
        self, client_id: str, symbols: List[str], room_id: str = "default"
    ):
        ...
        while self.is_streaming:
            try:
                ...
                for symbol in all_symbols:
                    ...
                    price_change = self._deterministic_gauss(
                        symbol, 0, self.market_volatility
                    )
                    ...
                    volume = self._deterministic_randint(
                        symbol, 100000, 10000000, seed=1
                    )
                ...
                sleep_time = (
                    2 + (int(time.time()) % 100) / 100.0 * 3
                )  # Deterministic 2-5 second range
                    ...
                    if symbol in self.price_cache:
                        ...
                        for room_id, room_data in self.active_subscriptions.items():
                            if symbol in room_data["symbols"]:
                                self.socketio.emit(
                                    "prediction_update", prediction_data, room=room_id
                                )

                # Update predictions every 30-60 seconds
                sleep_time = (
                    30 + (int(time.time()) % 100) / 100.0 * 30
                )  # Deterministic 30-60 second range
                time.sleep(sleep_time)
    ...
    def _get_base_price(self, symbol: str) -> float:
        ...
        return base_prices.get(
            symbol, self._deterministic_uniform(symbol, 50, 300, seed=999)
        )
    ...
    def get_market_summary(self) -> Dict[str, Any]:
        ...
        positive_movers = sum(
            1 for price in self.price_cache.values() if price > 100
        )  # Simplified
    ...
    def get_symbol_details(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            * self._deterministic_uniform(symbol, 1.01, 1.05, seed=2),
            "day_low": current_price
            * self._deterministic_uniform(symbol, 0.95, 0.99, seed=3),
            "opening_price": current_price
            * self._deterministic_uniform(symbol, 0.98, 1.02, seed=4),
            "volume": self._deterministic_randint(symbol, 1000000, 50000000, seed=5),
            "market_cap": current_price
            * self._deterministic_randint(symbol, 100000000, 3000000000, seed=6),
    ...
    def get_subscription_stats(self) -> Dict[str, Any]:
        ...
        total_clients = sum(
            len(room["clients"]) for room in self.active_subscriptions.values()
        )
