...
class TechnicalIndicators:
    ...
    def williams_r(
        ...
        for i in range(period - 1, len(close)):
            ...
            if window_high == window_low:
                ...
                williams = (
                    (window_high - close[i]) / (window_high - window_low)
                ) * -100
    ...
    def cci(
        ...
        typical_prices = np.array(
            [(h + l + c) / 3 for h, l, c in zip(high, low, close)]
        )
...
class IndicatorAnalyzer:
    ...
    def analyze_bollinger_bands(
        prices: List[float], upper: List[float], middle: List[float], lower: List[float]
    ) -> IndicatorResult:
        ...
        if current_price >= current_upper * 0.98:
            signal = "SELL"
            strength = (
                (current_price - current_middle) / (current_upper - current_middle)
            ) * 100
        # Price near lower band - potential buy
        elif current_price <= current_lower * 1.02:
            signal = "BUY"
            strength = (
                (current_middle - current_price) / (current_middle - current_lower)
            ) * 100
        else:
            signal = "HOLD"
    ...
    def get_composite_signal(indicators: List[IndicatorResult]) -> Dict[str, any]:
        ...
        agreement_count = (
            len(buy_signals) if overall_signal == "BUY" else len(sell_signals)
        )
