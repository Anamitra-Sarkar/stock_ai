...
class TestTechnicalIndicators:
    ...
    def sample_ohlc(self):
        ...
        for _ in range(50):
            # Generate a random change for the base price
            change = np.random.normal(0, 0.02)
            base_close = max(1.0, base_close * (1 + change))

            # High and low are derived from the close to ensure close is within range
            high = base_close * (1 + abs(np.random.normal(0, 0.01)))
            low = base_close * (1 - abs(np.random.normal(0, 0.01)))
    ...
    def test_atr_calculation(self, sample_ohlc):
        ...
        atr = TechnicalIndicators.atr(
            sample_ohlc["high"], sample_ohlc["low"], sample_ohlc["close"], 14
        )
