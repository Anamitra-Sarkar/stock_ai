...
class TestPredictionAgent:
    ...
    def test_generate_mock_data(self, agent):
        ...
        assert len(data) > 0
        assert all(
            key in data[0] for key in ["timestamp", "symbol", "close_price", "volume"]
        )
    ...
    def test_calculate_composite_confidence(self, agent):
        ...
        prediction = {"trend": "up", "base_confidence": 80}
        technical = {"technical_signal": "BUY", "technical_confidence": 75}
