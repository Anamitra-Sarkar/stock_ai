...
class RiskManager:
    ...
    def calculate_portfolio_risk(
        self, holdings: List[Dict[str, Any]], price_history: Dict[str, List[float]]
    ) -> RiskMetrics:
        ...
        try:
            ...
            sharpe_ratio = (
                (mean_return - self.risk_free_rate) / volatility
                if volatility > 0
                else 0
            )
            ...
            downside_deviation = (
                downside_returns.std().iloc[0] * np.sqrt(252)
                if len(downside_returns) > 0
                else 0
            )
            sortino_ratio = (
                (mean_return - self.risk_free_rate) / downside_deviation
            ...
            beta = self._calculate_beta(
                portfolio_returns.iloc[:, 0].values, market_returns
            )

            # Information Ratio and Tracking Error
            excess_returns = portfolio_returns.iloc[:, 0] - (
                market_returns / 252
    ...
    def _generate_market_returns(self, length: int) -> np.ndarray:
        ...
        return np.random.normal(
            0.08 / 252, 0.16 / np.sqrt(252), length
        )  # 8% annual return, 16% volatility
    ...
    def calculate_position_risks(
        self, holdings: List[Dict[str, Any]], total_portfolio_value: float
    ) -> List[PositionRisk]:
